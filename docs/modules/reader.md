...
**历史参考**：由 `docs/modules/reading.md`（插件形态设计）与 `docs/modules/reader.md`（宅桌面时期 Express 实现，已废弃）合并而来；本文件为唯一权威文档，Express/Drizzle 历史实现细节不再保留。
...
- **只读原则**：阅读器不修改源 TXT 文件，仅记录进度与书签（对应第 8 节验收 AC：源文件只读不被修改）。
...
存于 `globalStorage/reader.db`（只存路径与元数据，**不复制正文**）。
