...
**日期**：2026-05-30
**状态**：开发中（M1 已完成 — 文件导入）
**存储对齐**：本模块已纳入插件形态「模块目录」体系（设想②），详见 `common.md` 1.5；下文 Express/Prisma 方案为宅桌面时期历史实现，迁移至 VSCode 插件形态时以 Markdown/SQLite 索引公约重写。

---

## 0. 模块目录与存储（设想②）

- **模块目录**：TXT 源文件存放于 `{root}/reader/`（可配置文档保存目录下），用户可 `git` 跟踪或按 `.gitignore` 忽略大文件。
- **索引库**：阅读进度 / 编码 / 去重等元数据存于 `globalStorage/reader.db`（不进 Git，可重建，见 `common.md` ③）；正文不入库，仅存相对路径与 `file_hash` 去重。
- **只读原则**：阅读器不修改源 TXT 文件，仅记录进度与书签（对应 `reading.md` 验收 AC：源文件只读不被修改）。

> 注：当前 `docs/modules/reader.md` 为宅桌面时期方案，功能清单（R1~R18）仍适用，但技术栈需按插件形态调整；详见 `roadmap.md` S6。
