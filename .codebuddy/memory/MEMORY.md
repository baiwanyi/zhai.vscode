...
- 打包：`pnpm run package` = compile + `vsce package --no-dependencies`。**必须带 `--no-dependencies`**——vsce 内部用 `npm list` 检测生产依赖，pnpm 的符号链接布局会让它报一堆 `npm error missing: …` 并中止。当前 vsix 仅含 package.json / readme / dist / resources（`.vscodeignore` 忽略 `node_modules/**`），本地 F5 调试不受影响，但**发布前必须重新决定依赖携带策略**，否则装上的插件缺 `better-sqlite3` 与 `openai`。
